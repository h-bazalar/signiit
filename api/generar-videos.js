...
export default async function handler(req) {
  ...
  try {
    ...
    const n8nUrl = process.env.N8N_WEBHOOK_VIDEOS;
    if (!n8nUrl) return errorResponse("N8N_WEBHOOK_VIDEOS no configurado", 500);
    ...
    }).catch((err) => console.error("Error disparando n8n videos:", err));

    // Retornar inmediatamente con el id para polling
    return jsonResponse({ status: "processing", campanaId });
  } catch (err) {
    console.error("Error en generar-videos:", err);
